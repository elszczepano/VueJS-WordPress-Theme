<template>
  <v-container grid-list-md>
    <h2 class="display-2 main-page--header">
      <span class="text__red">N</span>ajnowsze artykuły
    </h2>
    <v-layout row wrap>
          ...
          <router-link :to="`/post/${slug}`">
            ...
            <v-card-title primary-title>
              <div>
                ...
                <div class="subheading">
                  {{
                    description | charReplace | sliceText(firstArticleLength)
                  }}...
                </div>
              </div>
            </v-card-title>
            <v-card-actions>
              ...
              <router-link :to="`/post/${slug}`"
                ><v-btn flat
                  >Czytaj dalej
                  <i class="material-icons red--marker">chevron_right</i></v-btn
                ></router-link
              >
            </v-card-actions>
          </router-link>
  ...
  mounted() {
    ...
    API.get("posts?per_page=3")
      ...
      .then(() => {
        ...
        this.thumbnail = this.articles[0]["better_featured_image"]?.[
          "source_url"
        ];
